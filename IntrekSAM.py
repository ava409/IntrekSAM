...
#TODO: Optimisation to speed up inference?
...
# Set the values based on RAM and VRAM limits, to avoid crashing.
# Setting VRAM_FRAME_LIMIT disables offloading frames to RAM, and increases the inference speed
# Setting RAM_FRAME_LIMIT disables loading all frames into RAM, and avoid crashing from running out of RAM
VRAM_FRAME_LIMIT = None
RAM_FRAME_LIMIT = 1500
...
class IntrekSAM_Video_Annotator(QtWidgets.QMainWindow):
    def __init__(self, ann_info_path, output_ann_dir, input_video_dir=None):
        super().__init__()
        self.setWindowTitle("IntrekSAM Video Annotator")
        self.setGeometry(50, 50, 800, 530)
        self.video_path = None
        self.ann_info_path = ann_info_path
        self.output_ann_dir = output_ann_dir
        self.input_video_dir = input_video_dir
        self.frame_size = (512, 288) #TODO: Currently limited to only to size of (512, 288). Need to dynamically change some things.
        self.cap = None
        self.play_fps = 60 # Target fps to run, but maxes out around 10 to 15 fps in practice
        self.playing = False        
        self.total_frames = 0    
    ...
    def init_obj(self):
        ...
        with open(self.ann_info_path, "r") as f:
    ...
    def init_ui(self):
        ...
        btn_load_frames = QtWidgets.QPushButton("Load Video Frames")
        btn_load_frames.clicked.connect(self.select_video_frames)
        btn_load_frames.setFocusPolicy(QtCore.Qt.NoFocus)
        button_layout.addWidget(btn_load_frames)
        btn_auto = QtWidgets.QPushButton("Load Auto")
        btn_auto.clicked.connect(self.select_auto)
        if self.input_video_dir is None:
    ...
    def select_video_frames(self):
        path = QtWidgets.QFileDialog.getExistingDirectory(self, "Select folder with video frames", "")
        if not path or len(os.listdir(path)) == 0:
            QtWidgets.QMessageBox.critical(self, "Error", "No folder selected or no files in selected folder.")
            return

        pattern = re.compile(r'^\d{10}\.(jpg|jpeg)$', re.IGNORECASE)
        for file in os.listdir(path):
            if not pattern.match(file):
                QtWidgets.QMessageBox.critical(self, "Error", "Invalid file format in the selected folder. Must only contain 10-digit zero padded filenames with jpg/jpeg ending. Example: 0000000000.jpg, 0000000001.jpg, ...")
                return
        self.video_path = path
        self.is_video = False
        self.vid_title_status.setText(f"<b>Folder Name:</b> {os.path.basename(self.video_path)}")
        self.load_video()


    def select_auto(self):
        input_list = [i.removesuffix(".mp4") for i in os.listdir(self.input_video_dir)]
        ann_list = os.listdir(self.output_ann_dir)
        to_ann_list = natsorted(list(set(input_list) - set(ann_list)))   
        self.video_path = os.path.join(self.input_video_dir, to_ann_list[0]+".mp4")
        self.is_video = True
        self.vid_title_status.setText(f"<b>Video Name:<b> {os.path.basename(self.video_path)}")
        self.btn_load_next.setEnabled(True)
        self.load_video()
    ...
    def load_video(self):
        ...
        if self.is_video:
            ...
            self.fps = str(round(self.cap.get(cv2.CAP_PROP_FPS), 2))
            if self.total_frames > RAM_FRAME_LIMIT:  
                QtWidgets.QMessageBox.warning(self, "Warning", f"Video has {self.total_frames} frames which may cause high RAM usage. Consider using the 'Load Video Frames' option or increasing the RAM_FRAME_LIMIT.")
                return
        else:
            frame_list = natsorted([f for f in os.listdir(self.video_path) if f.lower().endswith(('.jpg', '.jpeg'))])
            self.cap = frame_list
        ...
        offload_video_flag = True if VRAM_FRAME_LIMIT is None else self.total_frames > VRAM_FRAME_LIMIT
        sam2_video_path = self.video_path
        ...
        self.init_obj()
        self.goto_frame(0)
        self.frame_status.setText(f"Loaded {os.path.basename(self.video_path)} — {self.total_frames} frames @ {self.fps} fps")
    ...
    def goto_frame(self, idx: int, propagation: bool = False):
        ...
        if self.is_video:
            ...
            frame = cv2.imread(os.path.join(self.video_path, self.cap[idx]))
    ...
    def export_annotations(self):
        ...
        mask_dir = os.path.join(self.output_ann_dir, os.path.basename(self.video_path).split(".")[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ann_info_path", type=str, required=True)
    parser.add_argument("--output_ann_dir", type=str, required=True)
    parser.add_argument("--input_video_dir", type=str, required=False, default=None)
    args = parser.parse_args()   

    app = QtWidgets.QApplication(sys.argv)
    win = IntrekSAM_Video_Annotator(
        ann_info_path=args.ann_info_path,
